        ...
        timerHologram = new Hologram(timerLocation, ViewMode.ALL, RefreshMode.AUTOMATIC_FAST) {
            ...
            public List<String> getStrings(Player player) {
                ...
                if(AuctionManager.haveAuctionsEnded())
                    ...
                    strings.add("&e剩余时间: &f" + AuctionManager.getRemainingTime());
                return strings;
            }
        };
            ...
            bidInfoHolograms[i] = new Hologram(pedestalLocations[finalI].clone().add(0, 2, 0)) {
                ...
                public List<String> getStrings(Player player) {
                    ...
                    if(highestBidder != null)
                        strings.add("&e最高报价: &f" + highestBid + " 灵魂");
                    else
                        strings.add("&e初始报价: &f" + highestBid + " 灵魂");

                    String message = highestBidder == null ? "没有人!" : Bukkit.getOfflinePlayer(highestBidder).getName();
                    strings.add("&e来自: &6" + message);
                    strings.add("&e右键报价!");

...
public abstract class PitItem implements Listener {
    ...
    public boolean defaultUpdateItem(ItemStack itemStack) {
        ...
        if(hasLastServer) nbtItem.setString(NBTTag.ITEM_LAST_SERVER.getRef(), Thepit.status.name());
        return true;
    }
    ...
    public Thepit.ServerStatus getLastServer(ItemStack itemStack) {
        if(!isThisItem(itemStack) || !hasLastServer) throw new RuntimeException();
        NBTItem nbtItem = new NBTItem(itemStack);
        if(!nbtItem.hasKey(NBTTag.ITEM_LAST_SERVER.getRef())) return Thepit.ServerStatus.OVERWORLD;
        return Thepit.ServerStatus.valueOf(nbtItem.getString(NBTTag.ITEM_LAST_SERVER.getRef()));
    }

...
public class PlayerManager implements Listener {
    ...
    static {
        ...
        }.runTaskTimerAsynchronously(Thepit.INSTANCE, 0L, 1L);
        ...
        if(Thepit.getStatus().isOverworld()) {
            new BukkitRunnable() {
                @Override
                public void run() {
            ...
            }.runTaskTimer(Thepit.INSTANCE, 0L, 18L);
        }
    ...
    public static boolean isStaff(UUID uuid) {
        User user;
        try {
            user = Thepit.LUCKPERMS.getUserManager().loadUser(uuid).get();
        } catch(InterruptedException | ExecutionException exception) {
            exception.printStackTrace();
            return false;
        }
        Group group = Thepit.LUCKPERMS.getGroupManager().getGroup(user.getPrimaryGroup());
    ...
    public static void sendItemLossMessage(Player player, ItemStack itemStack) {
        ...
        TextComponent message = new TextComponent(ChatColor.translateAlternateColorCodes('&', "&c&l可惜!&7 你失去了 " +
...
//		message.addExtra(new TextComponent(ChatColor.translateAlternateColorCodes('&', "")));
    ...
    public static void sendLivesLostMessage(Player player, int livesLost) {
        ...
        AOutput.error(player, "&c&l可惜!&7 你在 &f" + livesLost + " &7个物品上失去了生命" + (livesLost == 1 ? "" : "s"));
    }
    ...
    public void onInteract(PlayerInteractAtEntityEvent event) {
        ...
        new PluginMessage().writeString("查看需求").writeString(player.getUniqueId().toString())
    ...
    public void onCommand(PlayerCommandPreprocessEvent event) {
        ...
        if(ChatColor.stripColor(event.getMessage()).toLowerCase().startsWith("/trade")) {
            ...
            if(pitPlayer.level < levelRequired) {
                event.setCancelled(true);
                AOutput.error(player, "&c&l错误!&7 你必须达到等级 " + levelRequired + " 才能进行交易");
                return;
            }
        }
        ...
        if(ChatColor.stripColor(event.getMessage()).toLowerCase().startsWith("/invsee")) {
            event.setCancelled(true);
            AOutput.send(player, "&c&l过时!&7 请改用 /view <玩家> 命令");
            return;
        }
    }
    ...
    public static void onKill(KillEvent killEvent) {
        ...
        if(Math.random() < 0.1 && killingNon == null && pitKiller.bounty < maxBounty) {
            ...
            String message = "&6&l悬赏!&7 提高了 &6&l" + bountyBump + "g&7 对 %luckperms_prefix%" + killEvent.getKillerPlayer().getDisplayName() +
                    "&7 的悬赏，以奖励高连击";
        ...
        if(kills == megastreak.requiredKills && !(megastreak instanceof NoMegastreak)) {
            ...
            for(Player onlinePlayer : Bukkit.getOnlinePlayers()) {
                ...
                String streakMessage = ChatColor.translateAlternateColorCodes('&',
                        "&c&l超级连击! %luckperms_prefix%" + pitPlayer.player.getDisplayName() + " &7激活了 " + megastreak.getCapsDisplayName() + "&7!");
            ...
            if(!multipleStacks) {
                if(firstArrow == -1) {
                    if(hasSpace) {
                        ...
                        AOutput.error(player, "请在你的背包中腾出空间以容纳箭矢");
                    }
                } else {
    ...
    public void onAttack(AttackEvent.Apply attackEvent) {
        ...
        if(PlayerManager.isRealPlayer(attackEvent.getDefenderPlayer()) && Thepit.status.isOverworld()) {
...
//		}
    ...
    public void onPlayerJoin(PitJoinEvent event) {
        ...
        if(Misc.isKyro(player.getUniqueId()) && Thepit.anticheat instanceof GrimManager) {
    ...
    public void onJoin(PlayerSpawnLocationEvent event) {
        Player player = event.getPlayer();
        PitPlayer pitPlayer = PitPlayer.getPitPlayer(player);
        Location spawnLoc = Thepit.getStatus() == Thepit.ServerStatus.DARKZONE ? MapManager.getDarkzoneSpawn() : MapManager.currentMap.getSpawn();
        ...
        if(ProxyMessaging.joinTeleportMap.containsKey(player.getUniqueId())) {
            ...
            new BukkitRunnable() {
                @Override
                public void run() {
                    if(!tpPlayer.isOnline()) AOutput.error(player, "&c你尝试传送的玩家不在线.");
                    else AOutput.send(player, "&a正在传送到 " + tpPlayer.getName() + "...");
                }
...
//					player.setVelocity(new Vector(1.5, 1, 0).multiply(0.3));
                    ...
                    AOutput.send(player, "&7你已被传送到 &d&k||&5&l黑暗区&d&k||&7。");
...
//						CutsceneManager.play(player);
                ...
                } else if(Thepit.getStatus() == Thepit.ServerStatus.OVERWORLD && LobbySwitchManager.joinedFromDarkzone.contains(player.getUniqueId()) &&
                    ...
                    Misc.sendSubTitle(player, "", 40);
                    AOutput.send(player, "&7你已被传送到 &a&l主世界&7。");
                }
    ...
    public void onJoin(AsyncPlayerPreLoginEvent event) {
        ...
        if(FirestoreManager.FIRESTORE == null) {
            event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER,
                    ChatColor.RED + "服务器仍在启动中");
        } else if (!success) {
            event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER,
                    ChatColor.RED + "玩家数据加载失败。请提交支持请求: discord.pitsim.net");
        }
        ...
        if(player.isOnline()) {
            event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER,
                    ChatColor.RED + "你已经在线了！\n如果你认为这是一个错误，请等待几秒钟后重新登录。");
        }
    }
    ...
    public void onBreak(BlockBreakEvent event) {
        ...
        event.setCancelled(true);
        AOutput.error(event.getPlayer(), "&c破坏方块功能已禁用，运行 /pitsim bypass 来切换");
    }
    ...
    public void onBreak(BlockPlaceEvent event) {
        ...
        if(toggledPlayers.contains(event.getPlayer())) return;
        event.setCancelled(true);
        AOutput.error(event.getPlayer(), "&c方块放置功能已禁用，运行 /pitsim bypass 切换此设置");
    }
    ...
    public void onItemFrameBreak(EntityDamageByEntityEvent event) {
        ...
        event.setCancelled(true);
        AOutput.error(player, "&c方块交互功能已禁用，运行 /pitsim bypass 切换此设置");
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if(toggledPlayers.contains(event.getPlayer())) return;
        event.setCancelled(true);
        AOutput.error(event.getPlayer(), "&c方块交互功能已禁用，运行 /pitsim bypass 切换此设置");
    }

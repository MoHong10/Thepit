...
import me.wisdom.thepit.commands.LightningCommand;
...
import me.wisdom.thepit.inventories.GenericConfirmationPanel;
...
import me.wisdom.thepitapi.libs.discord.DiscordWebhook;
...
public class Misc {
    ...
    public static void stunEntity(LivingEntity livingEntity, int ticks) {
        ...
        if(livingEntity instanceof Player) {
            ...
            sendTitle(player, "&c眩晕", ticks);
            sendSubTitle(player, "&e你不能移动！", ticks);
...
//	}
    ...
    public static void strikeLightningForPlayers(Location location, Player... players) {
        ...
        for(Player player : players) {
            ...
            player.playSound(location, Sound.ENTITY_GENERIC_EXPLODE, 10, (float) (Math.random() * 0.2 + 0.6));
        }
    }
...
//		wiji1
    ...
    public static String getRankColor(UUID uuid) {
        ...
        try {
            String rankColor = Thepit.LUCKPERMS.getUserManager().loadUser(uuid).get().getCachedData().getMetaData().getPrefix();
//			TODO: Check-in with players to make sure this fixed it and it wasn't returning "null" as a string
    ...
    public static String getRankString(UUID uuid) {
        try {
            String rank = Thepit.LUCKPERMS.getUserManager().loadUser(uuid).get().getCachedData().getMetaData().getPrimaryGroup();
            if(rank == null) throw new Exception();
            Group group = Thepit.LUCKPERMS.getGroupManager().getGroup(rank);

...
import me.wisdom.thepit.items.mobdrops.BlazeRod;
import me.wisdom.thepit.misc.CustomPitBlaze;
...
public class PitBlaze extends PitMob {
    public long lastAttack = Thepit.currentTick;
    ...
    public PitBlaze(Location spawnLocation, MobStatus mobStatus) {
        ...
        attackRunnable = new BukkitRunnable() {
            @Override
            public void run() {
                if(lastAttack + 40 > Thepit.currentTick) return;
                if(Math.random() > 0.25) return;

                Player target = getTarget();
                if(target == null) return;

                lastAttack = Thepit.currentTick;
                throwFireball(target);
            }
        }.runTaskTimer(Thepit.INSTANCE, new Random().nextInt(5), 5L);
    }
